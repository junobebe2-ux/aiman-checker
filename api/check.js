...
export default async function handler(req, res) {
  ...
  // Convert mozSpam (Moz v1: 0-17 scale) to percentage (0-100%)
  // Example: PrepPostSEO shows "SS" as 32% = mozSpam ~5-6
  const metrics = allResults.map(r => {
    const raw = parseFloat(String(r.mozSpam || "0").replace('%', '').trim()) || 0;
    // Konversi mozSpam v1 (0-17) ke persentase (0-100%)
    // Match DAPA: mozSpam=6 → SS=32. Formula: (raw / 18.75) * 100
    // Scale 0-18.75 biar 17 → 90 (bukan 100), agar lebih akurat di low-end
    let pct;
    if (raw <= 17) {
